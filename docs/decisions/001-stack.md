...
We use Next.js 15 (App Router) with TypeScript, deployed on Vercel Hobby. Product pages are statically generated at build time via `generateStaticParams`. We do **not** use `output: 'export'` because V1 needs API routes for analytics (`/api/track`) and a dynamic stats page (`/stats`).
